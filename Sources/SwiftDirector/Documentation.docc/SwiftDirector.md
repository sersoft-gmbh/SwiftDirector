...
Both methods optionally take a `port` parameter if you require a non-default port. If you need different schemes, there is the ``LDAPServer/init(schema:host:port:)`` initializer with which you can create a fully customized ``LDAPServer``.

### LDAPConnection

#### Binding and unbinding
Once you have a server, you need a connection. ``LDAPServer`` has a method named ``LDAPServer/openConnection()`` that will do exactly this for you:
```swift
...
Duplicated connections will destroy themselves when deallocated - leaving the original connection valid. There is also an explicit ``LDAPConnection/close()`` function which will destroy duplicated connections and unbind primary connections.
...
Ultimately, a connection can be used to search the directory. The ``LDAPConnection/search(for:inBase:filteredBy:)`` method is the way to go for that.
...
You can also simply check if the cast would succeed using ``LDAPObject/canCast(to:)``. And if for some reason you are **absolutely sure** that a given object can be casted to another object class, there is also ``LDAPObject/forceCast(to:)``. The latter should be treated similar to `try!` in Swift and only be used if absolutely necessary.
